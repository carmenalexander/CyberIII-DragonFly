...
class ShardArgs {
 ...
 public:
  struct Iterator {
    ...
    facade::CmdArgList args;
    absl::Span<const IndexSlice>::const_iterator it;
    uint32_t delta = 0;

    Iterator(facade::CmdArgList l, absl::Span<const IndexSlice>::const_iterator i)
        : args(l), it(i) {
    }

    bool operator==(const Iterator& o) const {
      return it == o.it && delta == o.delta;
    }
    ...
    std::string_view operator*() const {
      return facade::ArgS(args, it->first + delta);
    }
