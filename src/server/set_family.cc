...
OpResult<StringVec> OpUnion(const OpArgs& op_args, ShardArgs::Iterator beg,
                            ShardArgs::Iterator end) {
  DCHECK(beg != end);
  absl::flat_hash_set<string> uniques;

  for (; beg != end; ++beg) {
    auto find_res = op_args.shard->db_slice().FindReadOnly(op_args.db_cntx, *beg, OBJ_SET);
    if (find_res) {
...
OpResult<StringVec> OpDiff(const OpArgs& op_args, ShardArgs::Iterator beg,
                           ShardArgs::Iterator end) {
  DCHECK(beg != end);
  DVLOG(1) << "OpDiff from " << *beg;
  EngineShard* es = op_args.shard;
  auto find_res = es->db_slice().FindReadOnly(op_args.db_cntx, *beg, OBJ_SET);
  ...
  for (++beg; beg != end; ++beg) {
    auto diff_res = es->db_slice().FindReadOnly(op_args.db_cntx, *beg, OBJ_SET);
    if (!diff_res) {
...
OpResult<StringVec> OpInter(const Transaction* t, EngineShard* es, bool remove_first) {
  ...
  StringVec result;
  if (args.Size() == 1 + int(remove_first)) {
...
void SDiffStore(CmdArgList args, ConnectionContext* cntx) {
  ...
  auto diff_cb = [&](Transaction* t, EngineShard* shard) {
    ...
    DCHECK(!largs.Empty());
    ShardArgs::Iterator beg = largs.begin();
    ShardArgs::Iterator end = largs.end();
    if (shard->shard_id() == dest_shard) {
      CHECK_EQ(*beg, dest_key);
      ++beg;
      if (beg == end)
        return OpStatus::OK;
    }
    ...
    if (shard->shard_id() == src_shard) {
      CHECK_EQ(src_key, largs.Front());
      result_set[shard->shard_id()] = OpDiff(op_args, beg, end);  // Diff
    } else {
      result_set[shard->shard_id()] = OpUnion(op_args, beg, end);  // Union
    }
...
void SUnionStore(CmdArgList args, ConnectionContext* cntx) {
  ...
  auto union_cb = [&](Transaction* t, EngineShard* shard) {
    ...
    result_set[shard->shard_id()] = OpUnion(t->GetOpArgs(shard), beg, end);
    return OpStatus::OK;
  };
